...
def build():
    ...
    # The columns on the right that are the cat rather than its trail, for
    # anything that wants to place the cat and let the trail fall where it may.
    cat = sw - int(round((x0 + run - left) / step))
    palette = []
    index = {}
    out = []
    for (px, _), mask in zip(frames, masks):
        ...
        out.append(cells)
    flame(sw, sh, out)
    return sw, sh, cat, palette, out, [d for _, d in frames]
...
head = ["// Generated by nyan.py from assets/nyan.gif. Do not edit.", "#pragma once", "",
        "#include <stdint.h>", ""]
try:
    SW, SH, CAT, PAL, CELLS, DELAYS = build()
