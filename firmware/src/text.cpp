#include "text.h"

#include "board.h"

namespace {

constexpr int16_t GLYPH_W = 5;
constexpr int16_t GLYPH_H = 7;

// Column-major, one byte per column, bit 0 at the top. Space, then A-Z, then
// 0-9, then a full stop, a percent, a colon, an apostrophe, a hyphen and a
// plus - which between them is a mood, an address, a charge level, a clock, a
// label with its reading after it, whatever somebody has called their network,
// and a build made from a tree with changes in it.
const uint8_t GLYPHS[][GLYPH_W] = {
    {0x00, 0x00, 0x00, 0x00, 0x00}, {0x7E, 0x11, 0x11, 0x11, 0x7E},
    {0x7F, 0x49, 0x49, 0x49, 0x36}, {0x3E, 0x41, 0x41, 0x41, 0x22},
    {0x7F, 0x41, 0x41, 0x22, 0x1C}, {0x7F, 0x49, 0x49, 0x49, 0x41},
    {0x7F, 0x09, 0x09, 0x09, 0x01}, {0x3E, 0x41, 0x49, 0x49, 0x7A},
    {0x7F, 0x08, 0x08, 0x08, 0x7F}, {0x00, 0x41, 0x7F, 0x41, 0x00},
    {0x20, 0x40, 0x41, 0x3F, 0x01}, {0x7F, 0x08, 0x14, 0x22, 0x41},
    {0x7F, 0x40, 0x40, 0x40, 0x40}, {0x7F, 0x02, 0x0C, 0x02, 0x7F},
    {0x7F, 0x04, 0x08, 0x10, 0x7F}, {0x3E, 0x41, 0x41, 0x41, 0x3E},
    {0x7F, 0x09, 0x09, 0x09, 0x06}, {0x3E, 0x41, 0x51, 0x21, 0x5E},
    {0x7F, 0x09, 0x19, 0x29, 0x46}, {0x46, 0x49, 0x49, 0x49, 0x31},
    {0x01, 0x01, 0x7F, 0x01, 0x01}, {0x3F, 0x40, 0x40, 0x40, 0x3F},
    {0x1F, 0x20, 0x40, 0x20, 0x1F}, {0x3F, 0x40, 0x38, 0x40, 0x3F},
    {0x63, 0x14, 0x08, 0x14, 0x63}, {0x07, 0x08, 0x70, 0x08, 0x07},
    {0x61, 0x51, 0x49, 0x45, 0x43},
    {0x3E, 0x51, 0x49, 0x45, 0x3E}, {0x00, 0x42, 0x7F, 0x40, 0x00},
    {0x42, 0x61, 0x51, 0x49, 0x46}, {0x21, 0x41, 0x45, 0x4B, 0x31},
    {0x18, 0x14, 0x12, 0x7F, 0x10}, {0x27, 0x45, 0x45, 0x45, 0x39},
    {0x3C, 0x4A, 0x49, 0x49, 0x30}, {0x01, 0x71, 0x09, 0x05, 0x03},
    {0x36, 0x49, 0x49, 0x49, 0x36}, {0x06, 0x49, 0x49, 0x29, 0x1E},
    {0x00, 0x60, 0x60, 0x00, 0x00}, {0x23, 0x13, 0x08, 0x64, 0x62},
    {0x00, 0x66, 0x66, 0x00, 0x00}, {0x00, 0x03, 0x03, 0x00, 0x00},
    {0x00, 0x08, 0x08, 0x08, 0x00}, {0x08, 0x08, 0x3E, 0x08, 0x08},
};

constexpr uint8_t DIGIT_0 = 27;
constexpr uint8_t FULL_STOP = 37;
constexpr uint8_t PERCENT = 38;
constexpr uint8_t COLON = 39;
constexpr uint8_t APOSTROPHE = 40;
constexpr uint8_t HYPHEN = 41;
constexpr uint8_t PLUS = 42;

// Anything the font does not carry lands on the blank, which keeps a typo from
// walking off the end of the table.
inline const uint8_t *glyph(char c) {
  if (c >= 'a' && c <= 'z') {
    c = (char)(c - 'a' + 'A');
  }
  if (c >= 'A' && c <= 'Z') {
    return GLYPHS[c - 'A' + 1];
  }
  if (c >= '0' && c <= '9') {
    return GLYPHS[c - '0' + DIGIT_0];
  }
  if (c == '.') {
    return GLYPHS[FULL_STOP];
  }
  if (c == '%') {
    return GLYPHS[PERCENT];
  }
  if (c == '+') {
    return GLYPHS[PLUS];
  }
  if (c == ':') {
    return GLYPHS[COLON];
  }
  // The straight one only. A network named with the curly one is two bytes of
  // UTF-8 here and neither of them is a character this font has ever heard of.
  if (c == '\'') {
    return GLYPHS[APOSTROPHE];
  }
  if (c == '-') {
    return GLYPHS[HYPHEN];
  }
  return GLYPHS[0];
}

}  // namespace

int16_t textStep(int16_t scale) { return (int16_t)((GLYPH_W + 1) * scale); }

void textDraw(uint16_t *fb, const char *s, int16_t centreX, int16_t top, int16_t scale,
              uint16_t colour) {
  int16_t count = 0;
  for (const char *p = s; *p; p++) {
    count++;
  }
  if (count == 0 || scale < 1) {
    return;
  }

  int16_t step = textStep(scale);
  int16_t x = centreX - (count * step - scale) / 2;

  for (int16_t i = 0; i < count; i++) {
    const uint8_t *cols = glyph(s[i]);
    for (int16_t col = 0; col < GLYPH_W; col++) {
      uint8_t bits = cols[col];
      for (int16_t row = 0; row < GLYPH_H; row++) {
        if (!(bits & (1 << row))) {
          continue;
        }
        for (int16_t dy = 0; dy < scale; dy++) {
          int16_t at = top + row * scale + dy;
          if (at < 0 || at >= SCREEN_H) {
            continue;
          }
          uint16_t *line = boardRow(fb, at);
          for (int16_t dx = 0; dx < scale; dx++) {
            int16_t ax = x + col * scale + dx;
            if (ax >= 0 && ax < SCREEN_W) {
              line[boardX(ax)] = colour;
            }
          }
        }
      }
    }
    x += step;
  }
}
